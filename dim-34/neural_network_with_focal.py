...
# x_train, x_test= train_test_split(x_left, test_size=1/9, random_state = 20, shuffle=True)
# # y_train, y_test= train_test_split(y_left, test_size=1/9, random_state = 20, shuffle=True)
# y_train, y_test = train_test_split(y_onehot_left, test_size=1/9, random_state = 20, shuffle=True)
#
# # 将训练集进行合并，完成构建
# x_train = np.concatenate((x_index0, x_train), axis=0);
# y_train = np.concatenate((y_onehot_index0, y_train), axis=0);

# x_1 = np.arange(100); y_1 = x_1+1; z_1 = y_1 + 1 # 循环生成成功
# for i in range(0,10):
#     x_train, x_test= train_test_split(x_1, test_size=1/10, random_state = i ,shuffle=True)
#     y_train, y_test= train_test_split(y_1, test_size=1/10, random_state = i ,shuffle=True)
#     y_train_onehot, y_test_onehot = train_test_split(z_1, test_size=1/10, random_state = i ,shuffle=True)
#
#     print(x_test)
#     print(y_test)
#     print(y_test_onehot)

# build the focal loss function
...
for train_index, test_index in kf.split(range(0,126)):
    ...
    input_layer = Input((205)) # embedding size=100
    midlle_value = Dense(100, activation="relu")(input_layer)
    midlle_value = Dense(34, activation="relu")(midlle_value)
...
print("test acc std is: %f:" %acc_std)
...
with open('neural_network_focal_34_performance.pkl', 'wb') as file:
...
# with open('neural_network_focal_34_performance.pkl', 'rb') as file:
