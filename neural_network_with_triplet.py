...
# data generator for triplet loss function
...
def triplet_loss(y_true, y_pred, embedding_size = 100):
...
for train_index, test_index in kf.split(range(0,126)):
    ...
    class_model.compile(loss='categorical_crossentropy', optimizer="adam", metrics=['accuracy', f1])
    history = class_model.fit(x_train, y_train, epochs=50, verbose=2, validation_data=(x_test, y_test))
...
# with open('neural_network_triplet_performance.pkl', 'wb') as file:
