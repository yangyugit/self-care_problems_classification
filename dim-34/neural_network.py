...
for train_index, test_index in kf.split(range(0,126)):
    ...
    # for i in range(0, 10):
    #     x_train, x_test= train_test_split(x_left, test_size=1/9, random_state = i, shuffle=True)
    #     # y_train, y_test= train_test_split(y_left, test_size=1/9, random_state = 20, shuffle=True)
    #     y_train, y_test = train_test_split(y_onehot_left, test_size=1/9, random_state = i, shuffle=True)
    #
    #     # 将训练集进行合并，完成构建
    #     x_train = np.concatenate((x_index0, x_train), axis=0);
    #     y_train = np.concatenate((y_onehot_index0, y_train), axis=0);
...
with open('neural_network_34_performance.pkl', 'wb') as file:
...
with open('neural_network_34_performance.pkl', 'rb') as file:
